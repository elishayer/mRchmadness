...
#'   champion)
#' @author sspowers
sim.bracket.matrix = function(prob.matrix, num.reps, outcome, round, teams,
  untangling.indices) {

# Check that we have rows and columns in prob.matrix for all teams in bracket
  missing.rows = setdiff(teams, rownames(prob.matrix))
  if (length(missing.rows) > 0) {
    print(mRchmadness::teams %>% dplyr::filter(id %in% missing.rows))
    stop("prob.matrix has no rows for above teams. Row names must be team ID.")
  }
  missing.cols = setdiff(teams, colnames(prob.matrix))
  if (length(missing.cols) > 0) {
    print(mRchmadness::teams %>% dplyr::filter(id %in% missing.cols))
