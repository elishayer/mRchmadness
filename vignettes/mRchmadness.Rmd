...
2025-2026 season using the **scrape.game.results** function. In fact, you can
...
```{r, eval = FALSE}
games.2026 = scrape.game.results(2026, league = "women")
```
...
```{r, eval = FALSE}
pred.pop.2026 = scrape.population.distribution(year = 2026, league = "women")
```

We've made life even easier for you by pre-scraping **games.women.2026**,
**pred.pop.women.2026**, **games.men.2026** and **pred.pop.men.2026**. These are
all available as datasets in the package. You're welcome.

```{r}
head(games.women.2026)
head(pred.pop.women.2026)
```
...
```{r}
set.seed(1)
prob.matrix = bradley.terry(games = games.women.2026)
...
bracket in a vector called **bracket.women.2026**. This lists the tournament
teams in order of overall seed.

```{r}
head(bracket.women.2026)
```
...
set.seed(2026)
outcome = sim.bracket(
  bracket.empty = bracket.women.2026,
  prob.matrix = prob.matrix
)
draw.bracket(bracket.empty = bracket.women.2026, bracket.filled = outcome)
```

Congratulations to the winner of the `set.seed(2026)`
NCAA women's basketball tournament!

### Finding a good bracket

That's all well and good, but wouldn't you prefer to win
...
draw.bracket(bracket.empty = bracket.women.2026, bracket.filled = my.bracket)
```

### Testing your bracket

Let's see how well we can expect this
